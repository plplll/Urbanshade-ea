import { Eye, EyeOff, Copy, Download, Trash2, Info, AlertTriangle, AlertCircle, CheckCircle, Bug, Cpu } from "lucide-react";
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="p-3 border-b border-slate-800 flex items-center gap-2 flex-wrap bg-slate-900/50">
        ...
        <button 
          onClick={() => navigator.clipboard.writeText(logs.map(l => `[${l.type}] ${l.message}`).join("\n"))} 
          className="p-2 hover:bg-slate-800 rounded border border-slate-700 transition-colors"
          title="Copy logs"
        >
        ...
        {filteredLogs.length === 0 ? (
          ...
          filteredLogs.map(log => (
            <div key={log.id} className={`p-3 rounded-lg border transition-colors ${
              ...
              {showTechnical ? (
                <pre className="whitespace-pre-wrap break-all text-slate-300">{log.message}</pre>
              ) : (
                <div className="text-slate-300">{log.simplified || log.message}</div>
              )}
            </div>
          ))
        )}
